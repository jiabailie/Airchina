...
// Calculate a^b.
double
common::calPow(double base, ll exponent)
{
    if(exponent == 0) { return double(1); }
    if(exponent == 1) { return base; }
    if(exponent == 2) { return base * base; }

    double tmp = calPow(base, exponent >> 1);
    tmp = tmp * tmp;
...
common::imageSegment2Four(int width, int height, byte *data)
{
	...
	imageSegment(width, height, data, sample_rec_left_top, sample_rec_right_bottom);
}

void
common::imageSegment(int width, int height, byte *data, vector<point> &sample_rec_left_top, vector<point> &sample_rec_right_bottom)
{
	vector<int> rows;
	int i = 0, j = 0, ct = 0;	
